		...
		if(SYS_K<=SG_Time) 
		{
			if(SYS_K==SG_Time)//A event happen
			{
				if(SG_Index1==1)//Start to genrate continue segment
				{
					...
					if(Priority==5)
					{
						SG_Time +=(uint32_t)(Td/dT);
						Priority =3;
						Next_Priority =4;
					}
					else SG_Time +=(uint32_t)(SG.Td1/dT);
					SG_Index1 =2;
				}
				...
				else if(SG_Index1==4)
				{
					if(SG_Index==-1)//one cycle signals output finished
					{
						...
						else if((Next_Priority==1)||(Next_Priority==2)||(Next_Priority==3))//Switch warning level
						{
...
void Decode_UART_CMD(void)
{
	...
	switch(UART_RX_BUF[2])//Decode command package
	{
		case 0://Trigger high priority warning:0xaa 02 00 cc
		{
			...
			UART_TX_BUF[4] =0xcc;
			UART_TX_Len =5;
			if(Next_Priority==4)//040 140 240
			{
				if(Priority==1) Next_Priority =0;
				else if((Priority==2)||(Priority==3)) Next_Priority =1;
			}
			else
			{
		...
		case 1://Trigger medium priority warning:0xaa 02 01 cc
		{
			...
			UART_TX_BUF[4] =0xcc;
			UART_TX_Len =5;
			if(Next_Priority==4)//041 141 241
			{
				if(Priority==2) Next_Priority =0;
				else if((Priority==1)||(Priority==3)) Next_Priority =2;
			}
			else
			{
				if(Priority==0)//Trigger now
		...
		case 2://Trigger low priority warning:0xaa 02 02 cc
		{
			...
			UART_TX_BUF[4] =0xcc;
			UART_TX_Len =5;
			if(Next_Priority==4)//042 142 242
			{
				//if(Priority==3) Next_Priority =3;
				if((Priority==1)||(Priority==2)||(Priority==3)) Next_Priority =3;
			}
			else
			{
				if(Priority==0)//Trigger now
