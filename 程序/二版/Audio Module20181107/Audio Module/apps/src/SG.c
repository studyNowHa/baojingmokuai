...
typedef struct MY_SG
{
	...
	uint16_t Ts; /*Time between two pulse group*/
		...
		else if(Priority==2)//Flag for medium priority audio signal
		{
			...
			else 
			{
				...
				else 
				{
					SG.Ts =SG.Tb2*100;
					SG.Ts -=0.8*(SG.Tr+SG.Tf);
                    printf("SG.Ts.Tb2=%d",SG.Ts);
				}
				SG_Index =-1;
			}
		}
		else if((Priority==3)||(Priority==5))//Flag for Low priority audio signal or voice for key down
		{
			SG.Ts =10;
		}
	}
  if(Cal_Flag==2)
	{
		...
		if(SYS_K<=SG_Time) 
		{
			if(SYS_K==SG_Time)//A event happen
			{
				...
				{
					if((Priority==3)||(SG_Index==-1))//one cycle signals output finished
					{
						if(Next_Priority==6)
						{
							Next_Priority =0;
